#!/usr/bin/env python3
"""Measure vanilla Qwen HC-SMoE routing drift on contiguous held-out PG19 books."""
...
from hcsmoe.merging.sequential_drift_mixtral import freeze_for_analysis
from hcsmoe.merging.sequential_drift_qwen import assert_router_weights_unchanged, router_weight_snapshot
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--group-state-path", required=True, help="Saved vanilla Qwen group_state_dict.pt")
    parser.add_argument(
        "--dataset-name",
    ...
    parser.add_argument("--output-dir", default="results/routing_drift/qwen_hcsmoe_60to30_pg19")
...
def load_original_for_analysis(
    ...
    model = place_model_for_analysis(
        model,
        placement,
        no_split_module_classes=["Qwen2MoeDecoderLayer"],
        decoder_layer_name="Qwen2MoeDecoderLayer",
    )
...
def load_merged_for_analysis(
    ...
    if int(model.config.num_experts) != 60:
        raise AssertionError(f"expected the original 60-expert sparse router, got {model.config.num_experts}")
    assert_router_weights_unchanged(original_routers, model)
    print("[PG19 routing drift] Vanilla Qwen HC-SMoE router weights are bitwise identical")
    model = place_model_for_analysis(
        model,
        placement,
        no_split_module_classes=["Qwen2MoeDecoderLayer"],
    ...
    assert_router_weights_unchanged(original_routers, model)
    print_model_placement(model, "vanilla Qwen HC-SMoE")
...
def main() -> None:
    ...
    config = {
        ...
        "seed": args.seed,
        "comparison": "Original Qwen vs vanilla HC-SMoE 60-to-30",
        "router_comparison": "unchanged original sparse expert IDs; shared expert and merged group IDs are excluded",
