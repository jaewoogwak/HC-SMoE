...
class ExpertsGrouperForMixtral(object):
    ...
    def group_experts_by_clustering_output(
        ...
        for layer_idx in self.sparse_layer_indices:
            ffn_name = f"model.layers.{layer_idx}.block_sparse_moe"
            layer_input = torch.cat(forwarded_hidden_states[ffn_name], dim=0) # .cuda()
            expert_outputs = [] # (E, #T, D) -> average -> (E, D)
            ...
            with torch.no_grad():
                for i in range(self.num_experts):
                    ...
                    for start in range(0, layer_input.shape[0], chunk_size):
                        x = layer_input[start:start + chunk_size].to("cuda", non_blocking=True)
                        
                        y = expert(x)
                        output_sum += y.float().sum(dim=0)
                        num_tokens += y.shape[0]
                        
                        del x,y
                    
                    expert_mean = output_sum / num_tokens
                    expert_outputs.append(expert_mean.cpu())
                
                expert_outputs = torch.stack(expert_outputs).cuda()
