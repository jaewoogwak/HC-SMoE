...
def test_weighted_group_is_rebuilt_from_originals_and_router_is_unchanged():
    ...
    for name in ("w1", "w2", "w3"):
        expected = sum(originals[(index, name)] * usage[index] for index in (0, 1, 2)) / 6.0
        torch.testing.assert_close(getattr(layer.block_sparse_moe.experts[0], name).weight, expected)
    assert layer.block_sparse_moe.experts[0] is layer.block_sparse_moe.experts[1]
    assert layer.block_sparse_moe.experts[1] is layer.block_sparse_moe.experts[2]
